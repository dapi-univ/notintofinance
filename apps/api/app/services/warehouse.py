...
from app.schemas.warehouse import InstrumentMappingRecord
        ...
        *,
        ...
        async with self._repository.advisory_lock(
            ...
            for ticker in unique:
                output.append(await self._collect_symbol(ticker, trade_date, max_pages))
        return output

    async def _collect_symbol(
        self, ticker: str, trade_date: date, max_pages: int
    ) -> MicrostructureSymbolResult:
        mapping = await self._repository.mapping(ticker)
        if not mapping or mapping.mapping_status != "mapped" or not mapping.provider_instrument_id:
            return MicrostructureSymbolResult(ticker, 0, 0, 0, 0, "failed", "missing mapping")
        ...
        try:
            broker_rows, _ = await self._provider.get_broker_summary(
                ticker, instrument_id, trade_date
            )
            ...
            for _ in range(max_pages):
                page, _ = await self._provider.get_running_trades(
                    ticker, instrument_id, trade_date, cursor=cursor
                )
                pages += 1
            ...
            await self._repository.upsert_trade_prints(list(deduplicated.values()))

            snapshot, _ = await self._provider.get_orderbook(ticker, instrument_id)
            ...
            return MicrostructureSymbolResult(
                ...
                len(snapshot.levels),
                pages,
                "partial" if cursor else "succeeded",
            )
